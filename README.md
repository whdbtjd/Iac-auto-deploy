...
* 개발자는 자신이 개발한 프로젝트를 코드 푸시하는 것만으로도 인프라 생성부터 애플리케이션 배포까지 수행 가능
...
애플리케이션 배포 상태와 인프라 리소스 정보를 실시간으로 확인할 수 있는 모니터링 웹사이트를 시연용으로 구현하였습니다. Terraform outputs을 기반으로 AWS 리소스 상태를 시각화하여 배포가 정상적으로 완료되었는지 비교 검증할 수 있습니다. 실제로는 개발자가 프론트엔드와 백엔드 코드를 frontend/backend 디렉토리에 푸시하면, 자동으로 서버에 배포되도록 구성되어 있습니다.
